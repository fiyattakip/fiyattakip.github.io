...
// ===== Favori eşik bildirimi (varsayılan: %10 düşüş, 24 saat cooldown) =====
const FT_NOTIFY_PCT = 10;           // %10
const FT_NOTIFY_COOLDOWN_MS = 24 * 60 * 60 * 1000; // 24 saat
const _ftLastSeen = new Map();
...
function _ftCurrentPrice(fav){
  // Worker farklı alan isimleri yazabilir
  return _ftNum(fav.lastPrice ?? fav.price ?? fav.currentPrice ?? fav.last_price ?? null);
}

function _ftPrevPriceFromHistory(fav){
  const pts = Array.isArray(fav.priceHistory) ? fav.priceHistory : [];
  const arr = pts.map(p=>({t:Number(p?.t||0), p:_ftNum(p?.p)})).filter(x=>x.p!==null && Number.isFinite(x.t));
  if (arr.length < 2) return null;
  arr.sort((a,b)=>a.t-b.t);
  return arr[arr.length-2].p;
}

function _ftShouldNotify(id){
  const lastAt = Number(localStorage.getItem('ft_notif_at_' + id) || 0);
  return (Date.now() - lastAt) > FT_NOTIFY_COOLDOWN_MS;
}

function _ftMarkNotified(id){
  localStorage.setItem('ft_notif_at_' + id, String(Date.now()));
}

function _ftRememberPrice(id, price){
  if (price === null) return;
  localStorage.setItem('ft_last_price_' + id, String(price));
  _ftLastSeen.set(id, price);
}

function _ftLastRememberedPrice(id){
  if (_ftLastSeen.has(id)) return _ftLastSeen.get(id);
  const v = _ftNum(localStorage.getItem('ft_last_price_' + id));
  if (v !== null) _ftLastSeen.set(id, v);
  return v;
}

function _ftNotify(fav, prev, curr, pctDrop){
  const title = 'Fiyat düştü: ' + (fav.title || fav.query || 'Favori');
  const body = `${fav.siteName || ''} • ${prev} → ${curr} (-%${pctDrop.toFixed(1)})`;
  // Bildirim izni yoksa toast ile göster
  try{
...
function checkThresholdNotifications(favList){
  // sadece giriş varsa ve liste doluysa
  if (!Array.isArray(favList) || !favList.length) return;

  for (const fav of favList){
    const id = fav.id || fav._id;
    if (!id) continue;

    const curr = _ftCurrentPrice(fav);
    if (curr === null) continue;

    // prev price: history -> remembered
    const prevFromHist = _ftPrevPriceFromHistory(fav);
    const prev = prevFromHist ?? _ftLastRememberedPrice(id);
    if (prev === null || prev <= 0) { _ftRememberPrice(id, curr); continue; }

    // price changed?
    if (Math.abs(curr - prev) < 0.0001) { _ftRememberPrice(id, curr); continue; }

    const dropPct = ((prev - curr) / prev) * 100;
    if (dropPct >= FT_NOTIFY_PCT && _ftShouldNotify(id)){
      _ftNotify(fav, prev, curr, dropPct);
      _ftMarkNotified(id);
    }
    _ftRememberPrice(id, curr);
  }
}
// ===== /Favori eşik bildirimi =====
...
onAuthStateChanged(auth, async (u)=>{
  ...
  unsubFav = onSnapshot(qFav, (snap)=>{
    ...
    favCache = list;
    checkThresholdNotifications(favCache);
    renderFavorites();
  });
});
