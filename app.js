...
(function(){
  ...
  // Buton yoksa DOM'a ekle (temaya dokunmadan)
  function ensureAiButton(){
    if($("btnAiYorum")) return;
    var q = $("qNormal");
    if(!q) return;

    var btn = document.createElement("button");
    btn.id = "btnAiYorum";
    btn.className = "btnGhost full";
  ...
  async function handleAiYorum(){
    var q = $("qNormal")?.value?.trim();
    if(!q){ alert("Ürün adı gir"); return; }

    var box = $("aiYorumBox");
    if(box){
      box.innerText = "AI yorum hazırlanıyor...";
      box.classList.remove("hidden");
    }

    try{
      if(typeof window.geminiText === "function"){
        var res = await window.geminiText(
          "Bu ürün için kısa, net bir kullanıcı yorumu yap: " + q
        );
        if(box) box.innerText = res;
        else alert(res);
      }else{
        alert("AI fonksiyonu bulunamadı");
      }
    }catch(e){
  ...
  document.addEventListener("DOMContentLoaded", function(){
    ensureAiButton();
    var btn = $("btnAiYorum");
    if(btn) btn.addEventListener("click", handleAiYorum);
  });
})();
// ===== AI YORUM SON =====
